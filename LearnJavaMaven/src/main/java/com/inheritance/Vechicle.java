...
sealed abstract class Vehicle permits TwoWheeler {
    ...
    protected Vehicle(int wheels, String color, String model, long price) {
        if (wheels < 1) throw new IllegalArgumentException("Wheels must be >= 1"); // Fixed: realistic
    ...
    public String toString() {
        return "%s{model='%s', color='%s', price=%d}".formatted(getClass().getSimpleName(), model, color, price);
    }
}
